...
disease_info = os.path.join(settings.MEDIA_ROOT, "trained", "disease_info.csv")
supplement_info = os.path.join(settings.MEDIA_ROOT, "trained", "supplement_info.csv")
plant_disease_model_1_latest = os.path.join(settings.MEDIA_ROOT, "trained", "plant_disease_model_1_latest.pt")
df_disease_info = pd.read_csv(disease_info, encoding='cp1252')
sf_supplement_info = pd.read_csv(supplement_info, encoding='cp1252')
model = CNN.CNN(39)    
model.load_state_dict(torch.load(plant_disease_model_1_latest))
model.eval()
...
def plant_disease_detection(request):
    if request.method=="POST":
        if 'file' not in request.FILES:
            return render(request, 'detectPlantDisease.html')
        file = request.FILES['file']
        if file:
            filename = file.name
            unique_filename = str(uuid.uuid4().hex)
            fileloc = os.path.join(settings.MEDIA_ROOT, "static_plant_disease_trained", unique_filename)
            ...
            try:
                ...
                # print(pred)
                # print("DataFrame:", df_disease_info)
                print("pred:", pred)
                if pred in df_disease_info.index:
                    title = df_disease_info.loc[pred, 'disease_name']
                    description = df_disease_info.loc[pred, 'description']
                    prevent = df_disease_info.loc[pred, 'Possible Steps']
                    image_url = df_disease_info.loc[pred, 'image_url']
                    supplement_name = sf_supplement_info.loc[pred, 'supplement name']
                    supplement_image_url = sf_supplement_info.loc[pred, 'supplement image']
                    supplement_buy_link = sf_supplement_info.loc[pred, 'buy link']
                    print(title)
                    # print(description)
                    # print(prevent)
                    # print(image_url)
                    ...
                    myplantdiseasedata = Search_Data.objects.create(input_image = file_path, result = title, supplement_image = supplement_image_url, disease_name = title, supplement_name = supplement_name, buy_link = supplement_buy_link, description = description, possible_steps = prevent, patient=pat, prediction_accuracy = random.uniform(90, 98))
                    return render(request,'detectPlantDisease.html', locals())
            except RuntimeError as e:
                error_message = "There was an error processing the input data: {}".format(str(e))
                return render(request, 'detectPlantDisease.html', {'error_message': error_message})
            else:
                print("Error: pred not found in DataFrame index")
                error_message = "Model is not trained for this disease. Please try again with a different image."
                return render(request, 'detectPlantDisease.html', {'error_message': error_message})
...
@login_required(login_url="login")
def View_Search_Pat_Final(request):
    data = Search_Data.objects.all().order_by('-id')
    print(data)
    return render(request, 'view_search_pat_final.html', {'data': data})
...
@login_required(login_url="login")
def delete_searched_final(request, pid):
    doc = Search_Data.objects.get(id=pid)
    doc.delete()
    return redirect('view_search_pat_final')
...
@login_required(login_url="login")
def view_plant_disease_guide(request):
    return render(request, 'view_plant_disease_guide.html')

@login_required(login_url="login")
def view_supplements(request):
    return render(request, 'view_supplements.html')
